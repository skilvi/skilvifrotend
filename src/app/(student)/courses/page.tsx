...
export default async function CoursesPage({
  ...
  return (
    <div className="min-h-screen bg-[#f0f2f8] dark:bg-slate-950">
      {/* ── HERO SECTION ── */}
      <section className="bg-[#f0f2f8] dark:bg-slate-950 pt-20 pb-10">
      ...
      <CourseFilters initialQuery={query} initialCategory={category} initialSortBy={sortBy} />
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        ...
        {courses.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {courses.map((course) => (
              <ProgramCard key={course.id} course={course as any} />
            ))}
          </div>
        ) : (
          /* Empty state */
